 ...
 */
class CacheDelegate<T>(private val expirationTimeMillis: Long = 0, private val predicateExpired: ((T) -> Boolean)? = null) :
    ...
    override fun getValue(thisRef: Any?, property: KProperty<*>): T? {
        return (if (dataIsCachedAndNotExpired()) cached else null).also {
            System.out.println("delegate getValue: cached: $it")
        }
    }
